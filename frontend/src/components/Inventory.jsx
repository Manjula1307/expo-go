      ...
      {selectedCard && (
        <div style={styles.modalOverlay} onClick={() => setSelectedCard(null)}>
          <div style={styles.modalCard} onClick={(e) => e.stopPropagation()}>
            ...
            <h3 style={styles.cardTitle}>{selectedCard.name}</h3>
            <p style={styles.cardDescription}>{selectedCard.description}</p>
            <span style={{ ...styles.statusBadge, ...getStatusStyle(selectedCard.status) }}>
              {selectedCard.status}
            </span>
          </div>
        </div>
